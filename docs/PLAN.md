...
допустимом графом. Граф ацикличен (проверено при сборке плана).
...
- [ ] [R-08] Регистрация, вход, выход — задачи 04, 13, 14, 84 (приоритет M)
- [ ] [R-09] Подтверждение адреса, восстановление пароля — задачи 04, 13, 14 (приоритет M)
...
- [ ] [R-46] Очереди — задачи 09, 11, 74 (приоритет M)
...
| ОВ-25 срок ссылок подтверждения | 14 | значение из `settings`, по умолчанию 24 часа |
...
| ОВ-A3 сервис CAPTCHA | 14 | интерфейс `CaptchaVerifier`; реализация выбирается настройкой |
