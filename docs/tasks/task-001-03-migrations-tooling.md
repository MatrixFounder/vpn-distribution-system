...
- `control-plane/migrations/0001_extensions_roles_enums.sql` — `CREATE EXTENSION btree_gist, citext`; роли `app_owner`, `app_rw`, `app_migrate`, `app_backup`; все `enum`-типы §4.2
...
- точка входа выполняет `python -m app.cli migrate` при `APP_ROLE=api` до старта uvicorn (§10.2)
...
1. **TC-E2E-01:** Миграция применяется и откатывается
   ...
   - Ожидаемый результат: обе команды завершаются с кодом 0; после отката перечислений и ролей нет
