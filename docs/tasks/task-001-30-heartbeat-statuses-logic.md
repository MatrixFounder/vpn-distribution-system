...
- перевод ноды в `suspended` по повторным удержаниям частей отчёта за скользящие сутки
  (`held_anomaly`; порог числа удержаний — 001.35, алерт — 001.68): переход по матрице статусов
  §4.6 и возврат из него решением оператора — правило статуса здесь вместе с 001.35.
