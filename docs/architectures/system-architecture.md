...
- Правило границы 3: на `server` агентов `ssl_verify_client on`, а не `optional`, с глубиной
  проверки 1 — только листы, подписанные CA напрямую (лист, способный подписывать, множил бы
  отпечатки — ключи пределов).
- Правило границы 4: стоимость запроса до проверки identity в C-01 ограничивает прокси и
  проверка формы по байтам — на агентском и enrollment `server` тела без известной длины не
  принимаются (411 в едином формате при любом методе с `Transfer-Encoding` и при отсутствии
  длины у любого метода, кроме GET и HEAD),
  пределы тела по классам операций с буферами, равными пределам; C-01 сверяет тело каждой
  операции по байтам с формой её модели до разбора JSON — путь отказа не дороже пути приёма;
  на агентском `server` одновременность по отпечатку клиентского сертификата, частота по
  сертификату и на парк на уровне `server` (наследуется каждым `location`, включая пути
  отказа; отчёты — свои зоны в своём `location`; зоны парка — с постоянным ключом; остальным
  операциям — не больше четверти цикла событий процесса C-01, гейт в тестах); на
  enrollment-`server` (сертификата нет) пределы соединений и частоты по адресу источника и
  общий потолок частоты на уровне `server`, хеш токена — 001.72; прокси читает тело запроса
  целиком до передачи и ответ апстрима целиком без оглядки на клиента (умолчания объявлены
  явно — на них норматив Н-4 по времени апстрима), ответ сверх буферов уходит во временный файл
  на tmpfs не больше 8 МиБ на ответ, каталог временных файлов прокси — tmpfs под потолком памяти
  контейнера; отказы прокси — 411, 429 (с `Retry-After`) и 503 при недоступном апстриме (с
  `Retry-After`) — в едином формате ошибки §5.1, 413 — страница nginx.
